...
def train(train_loader, model, ema_m, criterion, optimizer, scheduler, epoch, args, logger):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            output, vis_embed, prototype = model(images)

            # if dist.get_rank() == 0:
            #     np.save('target.npy', target.cpu().detach().numpy())
            #     np.save('vis_embed.npy', vis_embed.cpu().detach().numpy())
            #     np.save('prototype_embed.npy', prototype.cpu().detach().numpy())
            ...
            features_vis_prototype = torch.cat([vis_embed.unsqueeze(1), prototype.unsqueeze(1)], dim=1)
            loss = criterion['aslloss'](output, target) + \
                   10*criterion['supcon'](features_vis_prototype, target)
